...
#: Metric names a family may emit that the profile deliberately does not declare.
#:
#: These are reported to a human reading one design; they are not scored. `build_metrics`
#: iterates `profile.metrics` and reads `raw_values.get(spec.name)`, so a name that is not
#: in the profile is simply never looked at — it displaces nothing and no declared metric
#: goes missing because of it. `ScoringProfile.spec` returning None is exactly the case
#: that supports this.
#:
#: That is what separates a diagnostic from the failure this rule exists to catch. A
#: *misspelling* ("leakage" for "predicted_leakage") means the intended metric is absent
#: and gets scored as the worst possible value. Adding a name here is a deliberate act, so
#: a typo — which nobody adds — still fails the test.
#:
#: Do not add a name here to silence the check. If the metric should influence the
#: ranking, it needs a MetricSpec in scoring/profiles.py instead.
DIAGNOSTIC_METRICS = frozenset(
    {
        # antisense.py: longest contiguous run of open positions in the initiation
        # region. Reported alongside the mean because a ribosome footprint needs a
...
#: The notebook workbench. `gates/notebooks/` is not package code — it has no
#: `__init__.py`, nothing under `engine/` imports from it, and it is run by hand in
#: Jupyter rather than by the pipeline. `_fixtures.py` builds tools *because* that is its
#: job: there is no `build_tools()` in a notebook to be handed one by. Exempt from the
#: shared-tool rule only; every other rule in this file still applies to it, including
#: the one that keeps `import RNA` behind FoldEngine.
NOTEBOOK_WORKBENCH = "engine/gates/notebooks/"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        if _rel(p) != TOOL_FACTORY and not _rel(p).startswith(NOTEBOOK_WORKBENCH)
    ],
    ids=_id,
)
...
def test_evaluate_design_emits_only_declared_metric_names(
    ...
    ``DIAGNOSTIC_METRICS`` is the deliberate exception — see its comment for why an
    *extra* name is not the same failure as a *misspelled* one.
    """
    declared = _declared_metric_names() | DIAGNOSTIC_METRICS
    ...
    assert not unknown, (
        ...
        + "\n\nThis is the signature of a misspelling: the name emitted here is dropped "
        "on the floor, and the declared metric it was meant to be is now missing, so "
        "missing_behavior scores it as the worst possible value. "
