...
<p><b>How far to close it is itself a design variable</b>, not a yes/no. Closing all three
positions is the most robust across candidates; closing two and leaving a designed
1&times;1 loop scores the same on the statistic we have evidence for and preserves the ON
state far better &mdash; but only on some candidates. Both belong in the panel, chosen per
candidate.
And note one thing not on the list at all: <b>our exposed toehold is a&nbsp;=&nbsp;0</b>,
outside the range anyone has tested. Kim&rsquo;s AND works at a&nbsp;=&nbsp;4 and degenerates
to a one-input switch at a&nbsp;=&nbsp;10. That is the parameter he varied deliberately, and
it may matter more than any of the five.</p>
</div>
...
every axis at once. Select on <code>A_M(10)</code>, <code>A_M(11)</code> <i>and</i> start
codon accessibility together, never on separation alone &mdash; the highest-separation rows
in every table here are the ones that never turn on.</li>
...
<li><b>Raise with the supervisor, as the largest open question:</b> a&nbsp;=&nbsp;0 puts us
outside the range anyone has tested. Kim&rsquo;s AND works at a&nbsp;=&nbsp;4 and becomes a
one-input switch at a&nbsp;=&nbsp;10, and it is the one parameter he varied deliberately &mdash;
if any single change deserves a construct of its own, it is a small non-zero toehold.
Separately: the equilibrium model cannot distinguish a validated AND gate from a switch that
does not gate, so for this architecture the bench is not a confirmation step &mdash; it is the
measurement.</li>
</ol>
