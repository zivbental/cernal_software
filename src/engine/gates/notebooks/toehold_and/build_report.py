...
<p>Then <b>select candidates where trigger A keeps exactly 2 of its 3 pairs at the start
codon</b> &mdash; that is the optimum of a real trade-off, measured below, not a preference.
...
candidates where <b>trigger A keeps exactly 2 of its 3 pairs</b> at the start codon (238
available), filtered on <code>A_M(11)</code> and start codon accessibility, never on
separation alone &mdash; the highest-separation rows are the ones that never turn on.</li>
