...
flowchart LR
    ...
    subgraph partial["Partly built"]
        ...
        P2["pipeline.py<br/><i>`direct` built; `de` built<br/>for E. coli, single-gene only</i>"]
        P3["switches.py<br/><i>sequence rules only</i>"]
        P4["PlasmidBuilder<br/><i>direct/single-switch/GFP/<br/>E. coli only — ROADMAP E5a</i>"]
        P5["TriggerScorer + S5<br/><i>real genes now too, E. coli —<br/>genes.md D6</i>"]
        P6["GeneSelector<br/><i>built, docs/genes.md —<br/>no count matrix in this<br/>product yet, so 3 of 5<br/>axes degrade to None</i>"]
    end

    subgraph todo["Step 5"]
        T1["S2–S5, S8, S9 tools"]
        T2["3 stage bodies<br/><i>quality, circuits,<br/>reporting</i>"]
        T3["ToeholdGate"]
...
| Supporting databases 5–8 | codon tables, sequence library, cell atlas, backbone spec | Sequence library ✅ for *E. coli* (`engine.transcriptome`, [genes.md](genes.md) D6); codon tables, cell atlas, backbone spec still **no data source chosen** |
| QC (S15) | `stages/quality.py` | ★ signature only — no count matrix exists in this product to check against ([genes.md](genes.md) §3 G-a) |
| Gene Selection (stage 1) | `stages/genes.py` | ✅ built and real ([genes.md](genes.md)) — tiered significance, effect-size floor/ceiling, directional abundance, no-folding trigger-yield screen, greedy non-redundancy, direction-balanced shortlist. Real for any host; wired into a real run only for *E. coli* today |
| Trigger Scoring (stage 2) | `stages/triggers.py` | ✅ `TriggerScorer.score` real and called from both paths — `direct` (ROADMAP.md E2b, a scanned paste) and now `de` too ([genes.md](genes.md) D6, *E. coli* only — real genes' real transcripts, not a synthesized single gene) |
...
| S5 | `stages/off_target.py` | ⚠️ empty-transcriptome case real (`scan_trigger`/`.scan_switch` give a defined, honest zero-penalty answer rather than raising — [ROADMAP.md](ROADMAP.md) E2b); a real transcriptome index (`build_index`, `find_similar`) is still its own stub — a real *E. coli* transcriptome now exists ([genes.md](genes.md) D6) but is deliberately not passed here yet, since populating it before `find_similar` is implemented would crash the run rather than degrade |
